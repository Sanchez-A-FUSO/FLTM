"""
多分类联邦训练（FedAvg）MLP 版本，便于与 CNN 版本做对比实验。
...
def build_model(
  ...
  return tf.keras.Model(inputs=inputs, outputs=outputs, name="federated_mlp")
...
def train_fedavg_keras(
  ...
  global_model = build_model(num_features, num_classes, dropout=dropout, hidden_units=hidden_units)
...
def train_fedavg_tff(
  ...
  trainer = tff.learning.algorithms.build_weighted_fed_avg(
      model_fn=model_fn,
      client_optimizer_fn=lambda: make_client_optimizer(client_opt, client_lr, clipnorm=clipnorm),
...
def main() -> None:
  parser = argparse.ArgumentParser(description="联邦学习 MLP 对比实验训练脚本")
  ...
  parser.add_argument(
      ...
      type=float,
      default=1.0,
      help="balanced 类权的幂次：<1 缓和稀有类权重，>1 放大稀有类权重；默认 1.0。",
  )
  parser.add_argument(
      "--dropout",
      type=float,
      default=0.0,
      help="隐藏层后 Dropout 比例，缓解过拟合；建议 0~0.3。",
  )
  parser.add_argument(
      "--hidden",
      type=str,
  ...
  parser.add_argument(
      ...
      help="crossentropy=稀疏交叉熵；focal=Focal Loss（难分样本权重更大）。",
  )
  parser.add_argument(
      "--focal-gamma",
      type=float,
  ...
  parser.add_argument(
      ...
      type=float,
      default=1.0,
      help="每轮客户端学习率乘子：第 r 轮 lr = client_lr * (round-lr-decay)^r；典型 0.98~1.0。",
  )
  parser.add_argument(
      "--clipnorm",
      type=float,
      default=0.0,
      help="客户端优化器全局梯度裁剪 L2 范数上限；>0 启用，例如 1.0。",
  )
